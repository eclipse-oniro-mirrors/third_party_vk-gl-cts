...
public:
	...
	{
		const uint32_t		height		= !initialSize ? (uint32_t)300 : initialSize->y();
		const uint32_t		width		= !initialSize ? (uint32_t)400 : initialSize->x();
		printf("%d,%d\n",width,height);
...
vk::wsi::Display* OhosPlatform::createWsiDisplay(vk::wsi::Type wsiType) const
{
    if(wsiType == vk::wsi::TYPE_OHOS){
        printf("ok\n");
		return new VulkanDisplayOhos();
    }
    return NULL;
}

vk::Library* OhosPlatform::createLibrary(LibraryType libraryType, const char* libraryPath) const
{
...
void OhosPlatform::getMemoryLimits(tcu::PlatformMemoryLimits& limits) const
{
