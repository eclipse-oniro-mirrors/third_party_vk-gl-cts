...
typedef VKAPI_ATTR VkResult				(VKAPI_CALL* CreateOHOSSurfaceOpenHarmonyFunc)										(VkInstance instance, const VkOHOSSurfaceCreateInfoOpenHarmony* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkSurfaceKHR* pSurface);
typedef VKAPI_ATTR VkResult				(VKAPI_CALL* GetOHOSNativeBufferPropertiesOpenHarmonyFunc)							(VkDevice device, const struct OH_NativeBuffer* buffer, VkOHOSNativeBufferPropertiesOpenHarmony* pProperties);
...
typedef VKAPI_ATTR VkResult				(VKAPI_CALL* GetSwapchainGrallocUsageOpenHarmonyFunc)								(VkDevice device, VkFormat format, VkImageUsageFlags imageUsage, int* grallocUsage);
typedef VKAPI_ATTR VkResult				(VKAPI_CALL* SetNativeFenceFdOpenHarmonyFunc)										(VkDevice device, int32_t nativeFenceFd, VkSemaphore semaphore, VkFence fence);
typedef VKAPI_ATTR VkResult				(VKAPI_CALL* GetNativeFenceFdOpenHarmonyFunc)										(VkQueue queue, uint32_t waitSemaphoreCount, const VkSemaphore* pWaitSemaphores, VkImage image, int32_t* pNativeFenceFd);
...
typedef VKAPI_ATTR VkResult				(VKAPI_CALL* GetMemoryOHOSNativeBufferOpenHarmonyFunc)								(VkDevice device, const VkMemoryGetOHOSNativeBufferInfoOpenHarmony* pInfo, struct OH_NativeBuffer** pBuffer);
