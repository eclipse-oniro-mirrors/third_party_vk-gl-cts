...
VkResult DeviceDriver::getSwapchainGrallocUsageOpenHarmony (VkDevice device, VkFormat format, VkImageUsageFlags imageUsage, int* grallocUsage) const
{
	return m_vk.getSwapchainGrallocUsageOpenHarmony(device, format, imageUsage, grallocUsage);
}

VkResult DeviceDriver::setNativeFenceFdOpenHarmony (VkDevice device, int32_t nativeFenceFd, VkSemaphore semaphore, VkFence fence) const
{
	return m_vk.setNativeFenceFdOpenHarmony(device, nativeFenceFd, semaphore, fence);
}

VkResult DeviceDriver::getNativeFenceFdOpenHarmony (VkQueue queue, uint32_t waitSemaphoreCount, const VkSemaphore* pWaitSemaphores, VkImage image, int32_t* pNativeFenceFd) const
{
	return m_vk.getNativeFenceFdOpenHarmony(queue, waitSemaphoreCount, pWaitSemaphores, image, pNativeFenceFd);
}
