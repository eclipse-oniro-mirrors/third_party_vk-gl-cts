 ...
 *//*!
 * \file
 * \brief OHOS Native Buffer Draw Tests
 *//*--------------------------------------------------------------------*/

#include "vktDrawOHOSNativeBufferTests.hpp"
...
struct DrawParams
{
	DrawParams (uint32_t numVertices, uint32_t numLayers)
...
class OHOSNativeBufferTestInstance : public TestInstance
{
public:
					OHOSNativeBufferTestInstance		(Context& context, DrawParams data);
					~OHOSNativeBufferTestInstance	    (void);
...
OHOSNativeBufferTestInstance::OHOSNativeBufferTestInstance (Context& context, DrawParams data)
...
OHOSNativeBufferTestInstance::~OHOSNativeBufferTestInstance (void)
{
}

void OHOSNativeBufferTestInstance::generateRefImage (const tcu::PixelBufferAccess& access,
                                                     ...
                                                     const vector<tcu::Vec4>& colors) const
{
	...
	renderer.draw(rr::DrawCommand(renderState, renderTarget, program, DE_LENGTH_OF_ARRAY(vertexAttribs),
								  &vertexAttribs[0], rr::PrimitiveList(rr::PRIMITIVETYPE_TRIANGLES,
                                  (uint32_t)vertices.size(), 0)));
}

class OHOSNativeBufferTestCase : public TestCase
{
	public:
					                OHOSNativeBufferTestCase(tcu::TestContext& context, const char* name,
                                                             const DrawParams data);
									~OHOSNativeBufferTestCase(void);
...
OHOSNativeBufferTestCase::OHOSNativeBufferTestCase (tcu::TestContext& context, const char* name, const DrawParams data)
...
OHOSNativeBufferTestCase::~OHOSNativeBufferTestCase	(void)
{
}

void OHOSNativeBufferTestCase::initPrograms (SourceCollections& programCollection) const
{
...
void OHOSNativeBufferTestCase::checkSupport (Context& context) const
{
...
void OHOSNativeBufferTestCase::initShaderSources (void)
{
...
TestInstance* OHOSNativeBufferTestCase::createInstance (Context& context) const
{
	return new OHOSNativeBufferTestInstance(context, m_data);
}

void OHOSNativeBufferTestInstance::generateDrawData (void)
{
...
tcu::TestStatus OHOSNativeBufferTestInstance::iterate (void)
{
	...
	const Unique<VkPipelineLayout>				pipelineLayout			(createPipelineLayout(vk, device,
	...
	for (uint32_t i = 0u; i < m_data.m_numLayers; i++)
	{
		...
		resultBufferAllocations.push_back(m_context.getDefaultAllocator().allocate(getBufferMemoryRequirements(vk,
                                          device, *resultBuffers.back()), MemoryRequirement::HostVisible));
	...
	const ImageCreateInfo						targetImageCreateInfo	(VK_IMAGE_TYPE_2D, colorAttachmentFormat,
                                                                         targetImageExtent, 1, m_data.m_numLayers,
	...
	if (!api)
		TCU_THROW(NotSupportedError, "OHOS Navite Buffer not supported");

	m_context.requireDeviceFunctionality("VK_OHOS_external_memory");

	uint64_t requiredUsage = api->VKUsageToOHOSUsage(VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);

    if (targetImageCreateInfo.arrayLayers > 1) {
        TCU_THROW(NotSupportedError, "Required number of layers for OHOS Native Buffer not supported");
    }

	pt::OHOSNativeBufferPtr bufferPtr = api->Allocate(WIDTH, HEIGHT,
                                                api->VKFormatToOHOSFormat(colorAttachmentFormat), requiredUsage);

	if (bufferPtr.internal == nullptr)
		TCU_THROW(NotSupportedError, "Required number of layers for OHOS Native Buffer not supported");

	NativeHandle			nativeHandle(bufferPtr);
	const Unique<VkImage>	colorTargetImage (createExternalImage(vk, device, queueFamilyIndex,
											  VK_EXTERNAL_MEMORY_HANDLE_TYPE_OH_NATIVE_BUFFER_BIT_OHOS,
                                              colorAttachmentFormat, WIDTH, HEIGHT, VK_IMAGE_TILING_OPTIMAL, 0u,
											  targetImageCreateInfo.usage, targetImageCreateInfo.mipLevels,
                                              targetImageCreateInfo.arrayLayers));

	uint32_t ohosFormat	= 0;
	api->Describe(nativeHandle.GetOhosNativeBuffer(), nullptr, nullptr, &ohosFormat, nullptr, nullptr);

	VkNativeBufferPropertiesOHOS	ohosProperties		=
	{
		VK_STRUCTURE_TYPE_NATIVE_BUFFER_PROPERTIES_OHOS,	            // VkStructureType    sType
		nullptr,														// void*              pNext
		0u,																// VkDeviceSize       allocationSize
		0u																// uint32_t           memoryTypeBits
	};

	vk.GetNativeBufferPropertiesOHOS(device, nativeHandle.GetOhosNativeBuffer(), &ohosProperties);

	const VkImportNativeBufferInfoOHOS	importInfo		=
	{
		VK_STRUCTURE_TYPE_IMPORT_NATIVE_BUFFER_INFO_OHOS,	            // VkStructureType            sType
		nullptr,														// const void*                pNext
		nativeHandle.GetOhosNativeBuffer()							    // struct AHardwareBuffer*    buffer
	};
	...
	const VkMemoryAllocateInfo					allocateInfo		=
	{
			...
			ohosProperties.allocationSize,					// VkDeviceSize       allocationSize
			chooseMemoryType(ohosProperties.memoryTypeBits)	// uint32_t           memoryTypeIndex
	};
	...
	PipelineCreateInfo::VertexInputState	vertexInputState		= PipelineCreateInfo::VertexInputState(1,
                                                                      &vertexInputBindingDescription, 2,
                                                                      vertexInputAttributeDescriptions);
	const VkDeviceSize						dataSize				= m_data.m_vertices.size() *
                                                                      sizeof(PositionColorVertex);
	de::SharedPtr<Buffer>					vertexBuffer			= Buffer::createAndAlloc(vk, device,
                                                                      BufferCreateInfo(dataSize,
																	  VK_BUFFER_USAGE_VERTEX_BUFFER_BIT),
                                                                      m_context.getDefaultAllocator(),
                                                                      MemoryRequirement::HostVisible);
	uint8_t*								ptr						= reinterpret_cast<uint8_t*>(
                                                                      vertexBuffer->getBoundMemory().getHostPtr());
	...
	const Unique<VkCommandBuffer>			cmdBuffer				(allocateCommandBuffer(vk, device, *cmdPool,
                                                                     VK_COMMAND_BUFFER_LEVEL_PRIMARY));
	const Unique<VkShaderModule>			vs						(createShaderModule(vk, device,
                                                                     m_context.getBinaryCollection().get("vert"), 0));
	const Unique<VkShaderModule>			fs						(createShaderModule(vk, device,
                                                                     m_context.getBinaryCollection().get("frag"), 0));
	...
	pipelineCreateInfo.addState(PipelineCreateInfo::ViewportState(1, vector<VkViewport>(1, viewport),
                                                                  vector<VkRect2D>(1, scissor)));
	...
	const VkImageMemoryBarrier				initialTransition		=
	{
		...
		*colorTargetImage,							// VkImage			image
		...
		makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0, m_data.m_numLayers)
	};

	vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                         (vk::VkDependencyFlags)0, 0, (const vk::VkMemoryBarrier*)nullptr, 0,
                         (const vk::VkBufferMemoryBarrier*)nullptr, 1, &initialTransition);
	...
	for (uint32_t i = 0; i < m_data.m_numLayers; i++)
	{
		...
		vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0u,
							  0u, nullptr, 1u, &bufferBarrier, 0u, nullptr);
	}
...
void CreateOHOSNativeBufferDrawTests (tcu::TestCaseGroup* testGroup)
{
	// Draw triangle list to a single layer color buffer
	testGroup->addChild(new OHOSNativeBufferTestCase(testGroup->getTestContext(), "triangle_list", DrawParams(9u, 1u)));

	// Draw triangle list to a color buffer with three layers
	testGroup->addChild(
        new OHOSNativeBufferTestCase(testGroup->getTestContext(), "triangle_list_layers_3", DrawParams(9u * 3u, 3u)));

	// Draw triangle list to a color buffer with five layers
	testGroup->addChild(
        new OHOSNativeBufferTestCase(testGroup->getTestContext(), "triangle_list_layers_5", DrawParams(9u * 5u, 5u)));

	// Draw triangle list to a color buffer with eight layers
	testGroup->addChild(
        new OHOSNativeBufferTestCase(testGroup->getTestContext(), "triangle_list_layers_8", DrawParams(9u * 8u, 8u)));

}

}

tcu::TestCaseGroup*	CreateOHOSNativeBufferTests(tcu::TestContext& testCtx)
{
	// Draw tests using OHOS Native Buffer
	return createTestGroup(testCtx, "ohos_native_buffer", CreateOHOSNativeBufferDrawTests);
}
