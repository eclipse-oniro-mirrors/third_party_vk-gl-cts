...
void NativeHandle::reset(void)
{
    ...
    m_hostPtr               = nullptr;
	m_ohosNativeBuffer		= vk::pt::OHOSNativeBufferPtr(nullptr);
                     ...
                     vk::VkExternalMemoryHandleTypeFlagBits externalType, NativeHandle &nativeHandle)
{
	...
	else if (externalType == vk::VK_EXTERNAL_MEMORY_HANDLE_TYPE_OH_NATIVE_BUFFER_BIT_OHOS)
	{
		...
		vk::pt::OHOSNativeBufferPtr						nativeBuffer(nullptr);
...
uint64_t OHOSNativeBufferExternalApi::MustSupportOHOSUsageFlags()
{
	return NATIVEBUFFER_USAGE_HW_TEXTURE | NATIVEBUFFER_USAGE_HW_RENDER;
}

uint64_t OHOSNativeBufferExternalApi::VKUsageToOHOSUsage(vk::VkImageUsageFlagBits vkFlags)
{
	switch(vkFlags)
	{
...
uint64_t OHOSNativeBufferExternalApi::VKCreateToOHOSUsage(vk::VkImageCreateFlagBits vkFlags)
{
	switch(vkFlags)
	{
...
vk::pt::OHOSNativeBufferPtr OHOSNativeBufferExternalApi::Allocate(uint32_t width,
																	   uint32_t height,
																	   uint32_t format,
																	   uint64_t usage)
{
...
uint32_t OHOSNativeBufferExternalApi::VKFormatToOHOSFormat(vk::VkFormat vkFormat)
{
	switch(vkFormat)
	{
