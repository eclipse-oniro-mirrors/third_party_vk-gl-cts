...
public:
	...
	uint64_t MustSupportOHOSUsageFlags();

	uint64_t VKUsageToOHOSUsage(vk::VkImageUsageFlagBits vkFlag);

	uint64_t VKCreateToOHOSUsage(vk::VkImageCreateFlagBits vkFlag);

	vk::pt::OHOSNativeBufferPtr Allocate(uint32_t width, uint32_t  height, uint32_t  format, uint64_t usage);

	uint32_t VKFormatToOHOSFormat(vk::VkFormat vkFormat);
