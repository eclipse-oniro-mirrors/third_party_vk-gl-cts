...
namespace vkt
{
	namespace RayTracing
	{
		namespace
		{
            ...
            static bool pointInTriangle2D(const tcu::Vec3 &p, const tcu::Vec3 &a, const tcu::Vec3 &b,
                const tcu::Vec3 &c)
			{
			...
			for (size_t testNdx = 0; testNdx < numTests; ++testNdx)
			{
				...
				for (size_t sizesNdx = 0; sizesNdx < DE_LENGTH_OF_ARRAY(sizes); ++sizesNdx)
				{
                    ...
                    const uint32_t randomSeed =
                        (uint32_t)(5 * testNdx + 11 * sizes[sizesNdx]) +
                        (uint32_t)testCtx.getCommandLine().getBaseSeed();
